 ...
 * Register a new user — account is created as INACTIVE and requires admin approval.
 */
router.post(
  "/register",
  authLimiter,
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const response: ApiResponse = {
      success: true,
      message: "בקשת ההרשמה נשלחה. חשבונך ממתין לאישור מנהל המערכת.",
    };
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!user.isActive) {
      throw new AppError("החשבון ממתין לאישור מנהל המערכת", 403);
    }
