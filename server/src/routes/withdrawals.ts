...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    const { supplierId, status, frameworkOrderId, fromDate, toDate } =
      req.query;
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (frameworkOrderId) {
      ...
      if (!frameworkOrder) {
        ...
        Array.isArray(frameworkOrder.items) &&
        frameworkOrder.items.length === 0
      ) {
        const order = await prisma.order.findUnique({
      ...
      // C1: Validate requested quantities against framework order available quantities
      const frameworkItems = await prisma.frameworkOrderItem.findMany({
        where: { frameworkOrderId: resolvedFrameworkOrderId },
        include: { reagent: { select: { name: true } } },
      });
      for (const item of items) {
        const fwItem = frameworkItems.find(
          (fi: any) => fi.reagentId === item.reagentId,
        );
        if (fwItem) {
          const available = Number(fwItem.availableQuantity) || 0;
          const requested = Number(item.requestedQuantity) || 0;
          if (requested > available) {
            throw new AppError(
              `הכמות המבוקשת (${requested}) עבור ${fwItem.reagent?.name || item.reagentId} חורגת מהכמות הזמינה במסגרת (${available})`,
              400,
            );
          }
        }
      ...
      async (tx) => {
        ...
        if (lastWithdrawal) {
          const lastNum = parseInt(
            lastWithdrawal.withdrawalNumber.replace("WD-", ""),
          );
        ...
        return tx.withdrawalRequest.create({
          data: {
            ...
            totalValueRequested:
              totalValueRequested > 0 ? totalValueRequested : null,
            items: {
...
router.put(
  "/:id",
  validateBody(updateWithdrawalSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const {
      requesterNotes,
    ...
    if (existing.status !== "DRAFT") {
      throw new AppError(
        "Can only update withdrawal requests in DRAFT status",
        400,
      );
    }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existing.status !== "DRAFT") {
      throw new AppError(
        "Can only submit withdrawal requests in DRAFT status",
        400,
      );
    }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existing.status !== "SUBMITTED") {
      throw new AppError(
        "Can only approve withdrawal requests in SUBMITTED status",
        400,
      );
    }
    ...
    for (const item of updatedItems) {
      if (item.unitPrice && item.approvedQuantity) {
        totalValueApproved +=
          Number(item.unitPrice) * Number(item.approvedQuantity);
      }
    }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existing.status !== "SUBMITTED") {
      throw new AppError(
        "Can only reject withdrawal requests in SUBMITTED status",
        400,
      );
    }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existing.status !== "SHIPPING" && existing.status !== "APPROVED") {
      throw new AppError(
        "Can only complete withdrawal requests in APPROVED or SHIPPING status",
        400,
      );
    }
    ...
    const withdrawal = await prisma.$transaction(async (tx) => {
      ...
      const deliveryItems = updatedItems.filter(
        (item) => (Number(item.fulfilledQuantity) || 0) > 0,
      );
      if (deliveryItems.length > 0) {
        await tx.delivery.create({
          data: {
            ...
            status: "NEW",
            notes: `Auto-created from withdrawal ${existing.withdrawalNumber}. יש למלא מספר אצווה ותוקף בעת קליטת המשלוח.`,
            items: {
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existing.status === "CLOSED" || existing.status === "CANCELLED") {
      throw new AppError(
        "Cannot cancel completed or already cancelled withdrawal request",
        400,
      );
    }
...
router.post(
  "/:id/items",
  validateBody(addWithdrawalItemSchema),
  asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    if (existing.status !== "DRAFT") {
      throw new AppError(
        "Can only add items to withdrawal requests in DRAFT status",
        400,
      );
    }
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existing.status !== "DRAFT") {
      throw new AppError(
        "Can only update items in withdrawal requests in DRAFT status",
        400,
      );
    }
...
router.delete(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (existing.status !== "DRAFT") {
      throw new AppError(
        "Can only remove items from withdrawal requests in DRAFT status",
        400,
      );
    }
