...
const calculateSuggestionsLogic = (
  ...
  return reagentsToProcess.map((reagent) => {
    const batchesForReagent = (allBatches || []).filter(
      (b) => b.reagent_id === reagent.id,
    );
    const currentStock = batchesForReagent.reduce(
      (sum, b) => sum + (b.current_quantity || 0),
      0,
    );

    const nearExpiryBatchesForReagent = batchesForReagent.filter(
      (b) =>
        b.expiry_date && isBefore(parseISO(b.expiry_date), nearExpiryLimit),
    );
    const nearExpiryQuantity = nearExpiryBatchesForReagent.reduce(
      (sum, b) => sum + (b.current_quantity || 0),
      0,
    );

    const frameworkOrderItems = (allFrameworkOrderItems || []).filter(
      (item) => item.reagent_id === reagent.id,
    );
    ...
    const detailedFrameworkItems = frameworkOrderItems
      .map((item) => ({
        orderNumber:
          (allOpenOrders || []).find((o) => o.id === item.order_id)
            ?.order_number_permanent || "N/A",
    ...
    const effectiveMonthlyUsage =
      reagent.use_manual_usage && reagent.manual_monthly_usage > 0
        ? reagent.manual_monthly_usage
        : reagent.average_monthly_usage || 0;
    const months_of_stock =
      effectiveMonthlyUsage > 0
        ? currentStock / effectiveMonthlyUsage
        : Infinity;
...
export default function InventoryReplenishmentPage() {
  ...
  const [availableFrameworkFilter, setAvailableFrameworkFilter] =
    useState("all");
  ...
  const [sortConfig, setSortConfig] = useState(() => {
    try {
      const s = localStorage.getItem("inventoryReplenishment_sortConfig");
      return s ? JSON.parse(s) : { key: "name", direction: "asc" };
    } catch {
  ...
  useEffect(() => {
    localStorage.setItem(
      "inventoryReplenishment_sortConfig",
  ...
  const [pendingWithdrawalByReagentState, setPendingWithdrawalByReagentState] =
    useState({});
  const [inDeliveryByReagentState, setInDeliveryByReagentState] = useState({});
  const [quantityInTransitByReagentState, setQuantityInTransitByReagentState] =
    useState({});
  const [
    quantityInTransitWithoutTempByReagentState,
    setQuantityInTransitWithoutTempByReagentState,
  ] = useState({});

  const [showMissingDetailsDialog, setShowMissingDetailsDialog] =
    useState(false);
  ...
  const [columnWidths, setColumnWidths] = useState(() => {
    try {
      const stored = localStorage.getItem(
        "inventoryReplenishment_columnWidths",
      );
      if (stored) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!errorMessage) {
        ...
        setAllOpenOrderItems(
          Array.isArray(openOrderItemsData) ? openOrderItemsData : [],
        );
        setAllPendingWithdrawalItems(
          Array.isArray(withdrawalRequestsData) ? withdrawalRequestsData : [],
        );
        setAllOpenOrders([
          ...(Array.isArray(openOrdersData) ? openOrdersData : []),
          ...(Array.isArray(frameworkOrdersData) ? frameworkOrdersData : []),
        ]);
        setAllFrameworkOrderItems(
          Array.isArray(frameworkOrderItemsData) ? frameworkOrderItemsData : [],
        );
        ...
        throw new Error(
          errorMessage || "No data returned from server function.",
        );
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (filter === "suggested")
      result = result.filter((r) => r.suggested_order_quantity > 0);
    else if (filter === "low_stock")
      result = result.filter(
        (r) =>
          r.current_stock_status === "low_stock" ||
          r.current_stock_status === "out_of_stock",
      );
    else if (filter === "out_of_stock")
      result = result.filter((r) => r.current_stock_status === "out_of_stock");

    if (categoryFilter !== "all")
      result = result.filter((r) => r.category === categoryFilter);
    if (supplierFilter !== "all")
      result = result.filter((r) => r.supplier?.name === supplierFilter);

    if (availableFrameworkFilter === "with_framework") {
      result = result.filter((r) => {
        const netAvailable =
          (r.available_framework_quantity || 0) -
          (r.quantity_in_pending_withdrawals || 0) -
          (r.quantity_in_delivery || 0);
    ...
    categoryFilter,
    supplierFilter,
    availableFrameworkFilter,
    processedReagents,
    sortConfig,
  ]);

  const handleSelectReagent = useCallback(
    (reagentId, initialSuggestedQuantity) => {
      setSelectedReagents((prev) => {
        ...
        if (existingItem) {
          ...
          const currentQuantity =
            (processedReagents || []).find((r) => r.id === reagentId)
              ?.suggested_order_quantity || initialSuggestedQuantity;
  ...
  const handleSelectAllFiltered = useCallback(() => {
    setSelectedReagents((prev) => {
      if (
        prev.size === filteredReagents.length &&
  ...
  const handleQuantityChange = useCallback((reagentId, newQuantity) => {
    ...
    setProcessedReagents((prevReagents) =>
      (prevReagents || []).map((r) =>
        r.id === reagentId
          ? { ...r, suggested_order_quantity: parsedQuantity }
          : r,
      ),
    );
  ...
  const createDocument = async (documentType) => {
    const items = Array.from(selectedReagents)
      .map((selected) => {
        const reagent = (processedReagents || []).find(
          (r) => r.id === selected.id,
        );
    ...
    if (items.length === 0) {
      return toast({
        ...
        description: "יש לבחור לפחות פריט אחד עם כמות גדולה מ-0.",
        variant: "destructive",
      });
    }

    const supplierKeys = items
      .map((item) => {
    ...
    if (uniqueSupplierKeys.length > 1) {
      return toast({
        title: "ספקים מרובים",
        description: "יש לבחור פריטים מספק אחד בלבד.",
        variant: "destructive",
      });
    }

    if (documentType === "withdrawal") {
      await handleWithdrawalCreation(
        items,
        items[0]?.supplier || uniqueSupplierKeys[0],
      );
    } else {
  ...
  const handleWithdrawalCreation = async (items, supplier) => {
    try {
      ...
      for (const item of items) {
        const processedReagent = (processedReagents || []).find(
          (r) => r.id === item.id,
        );
        const netAvailable =
          (processedReagent?.available_framework_quantity || 0) -
          (processedReagent?.quantity_in_pending_withdrawals || 0) -
          (processedReagent?.quantity_in_delivery || 0);
      ...
      const relevantFrameworkOrders = (allOpenOrders || []).filter((o) => {
        ...
        const matchesSupplierId =
          supplierId && orderSupplierId && orderSupplierId === supplierId;
        const matchesSupplierName =
          supplierNameLower &&
          orderSupplierName &&
          orderSupplierName === supplierNameLower;
        return (
          (o.order_type === "framework" ||
            o.order_type === "framework_order") &&
      ...
      for (const order of relevantFrameworkOrders) {
        const canSupplyAll = items.every((itm) => {
          const fwItem = (allFrameworkOrderItems || []).find(
            (foi) => foi.order_id === order.id && foi.reagent_id === itm.id,
          );
          if (!fwItem) return false;
          const remaining =
            (fwItem.quantity_ordered || 0) - (fwItem.quantity_received || 0);
      ...
      const hasPermanentNumber =
        selectedFrameworkOrder.order_number_permanent &&
        selectedFrameworkOrder.order_number_permanent.trim() !== "";
      const hasPONumber =
        selectedFrameworkOrder.purchase_order_number_sap &&
        selectedFrameworkOrder.purchase_order_number_sap.trim() !== "";
  ...
  const executeWithdrawalCreation = async (frameworkOrderId, items) => {
    try {
      ...
      if (success) {
        ...
        throw new Error(
          errorMessage || payload.error || "Failed to create withdrawal",
        );
      }
    } catch (error) {
      toast({
  ...
  const columns = useMemo(
    () => [
      { id: "selection", header: "", width: 50, disableSort: true },
      {
        ...
        header: "שם פריט",
        width: 250,
        description: COLUMN_DESCRIPTIONS.inventoryReplenishment.name,
      },
      {
        id: "effective_monthly_usage",
        header: "צריכה חודשית",
        width: 120,
        description:
          COLUMN_DESCRIPTIONS.inventoryReplenishment.effective_monthly_usage,
      },
      {
        id: "total_quantity_all_batches",
        header: "מלאי נוכחי",
        width: 120,
        description:
          COLUMN_DESCRIPTIONS.inventoryReplenishment.total_quantity_all_batches,
      },
      {
        id: "months_of_stock",
        header: "חודשי מלאי",
        width: 120,
        description: COLUMN_DESCRIPTIONS.inventoryReplenishment.months_of_stock,
      },
      {
        id: "available_framework_quantity",
        header: "יתרות זמינות",
        width: 120,
        description:
          COLUMN_DESCRIPTIONS.inventoryReplenishment
            .available_framework_quantity,
      },
      {
        id: "suggested_order_quantity",
        header: "כמות מוצעת",
        width: 150,
        description:
          COLUMN_DESCRIPTIONS.inventoryReplenishment.suggested_order_quantity,
      },
    ],
    [],
  );
  ...
  const itemsWithTemporaryOrders = useMemo(() => {
    return (filteredReagents || []).filter((r) => r.has_temporary_orders)
      .length;
  }, [filteredReagents]);
  ...
  const availableCategories = useMemo(
    () =>
      [
        ...new Set(
          (processedReagents || []).map((r) => r.category).filter(Boolean),
        ),
      ].sort(),
    [processedReagents],
  );
      ...
      <div className="p-4 md:p-6 min-h-screen bg-slate-50" dir="rtl">
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  <p className="text-sm text-slate-500">
                    מציג {filteredReagents.length} פריטים |{" "}
                    {selectedReagents.size} נבחרו
                  </p>
                  {itemsWithTemporaryOrders > 0 && (
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        >
                          <p className="text-sm">
                            {itemsWithTemporaryOrders} פריטים מוצגים עם כמות
                            מוצעת המתחשבת בדרישות רכש זמניות (ללא מספר קבוע).
                            המספר בסוגריים מציג את הכמות ללא התחשבות בדרישות
                            אלו.
                          </p>
                        </TooltipContent>
                      </Tooltip>
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <Button
                onClick={() => createDocument("withdrawal")}
                disabled={selectedReagents.size === 0}
                variant="outline"
                className="bg-blue-600 hover:bg-blue-700 text-white h-9"
              >
                <ArrowDownToLine className="h-4 w-4 ms-2" /> צור משיכה
              </Button>
              <Button
              ...
              >
                <FileDown className="h-4 w-4 ms-2" /> צור דרישה
              </Button>
            </div>
          </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <Select
                          value={availableFrameworkFilter}
                          onValueChange={setAvailableFrameworkFilter}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="כל הפריטים" />
                          </SelectTrigger>
                          <SelectContent>
                            ...
                            <SelectItem value="without_framework">
                              ללא יתרות זמינות
                            </SelectItem>
                          </SelectContent>
                        </Select>
                        ...
                        <div>
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="months_of_stock-asc">
                                חודשי מלאי (נמוך-גבוה)
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
        ...
        {isMobile ? (
          <div className="space-y-3 pb-24">
            {(filteredReagents || []).map((reagent) => {
              const netAvailable =
                (reagent.available_framework_quantity || 0) -
                (reagent.quantity_in_pending_withdrawals || 0) -
                (reagent.quantity_in_delivery || 0);
              return (
                <Card
                  key={reagent.id}
                  className={`p-3 border-s-4 bg-white transition-all ${
                    ...
                    <div className="flex-grow">
                      <div className="text-center mb-2">
                        <SmartTooltip
                          ...
                          className="font-bold text-base text-slate-800 block"
                          isMobile={true}
                        >
                        ...
                        <SmartTooltip
                          content={`${reagent.catalog_number} | ${reagent.supplier?.name || ""}`}
                      ...
                      <div className="grid grid-cols-2 gap-2 text-center text-xs">
                        <div className="bg-slate-50 p-1.5 rounded-lg">
                          ...
                          <p className="font-semibold text-slate-800 text-sm">
                            {formatQuantity(reagent.total_quantity_all_batches)}
                          </p>
                        </div>
                        <div className="bg-blue-50 p-1.5 rounded-lg">
                          <Popover>
                            <PopoverTrigger asChild>
                              <button
                                className="w-full text-blue-600 hover:text-blue-800 underline disabled:text-slate-400 disabled:no-underline"
                                disabled={
                                  !(
                                    reagent.available_framework_quantity ||
                                    reagent.quantity_in_pending_withdrawals ||
                                    reagent.quantity_in_delivery
                                  )
                                }
                              >
                                <Label className="text-xs font-medium text-blue-600 underline block">
                                  יתרות זמינות
                                </Label>
                            ...
                            >
                              <div className="space-y-2 text-sm">
                                ...
                                <div className="text-xs text-slate-500 mt-2 pt-2 border-t border-slate-100">
                                  <p>
                                    💡 יתרה נטו = יתרות מסגרת מינוס משיכות
                                    ממתינות ומינוס משיכות באספקה
                                  </p>
                                </div>
                              </div>
                            </PopoverContent>
                        ...
                        <div className="bg-slate-50 p-1.5 rounded-lg">
                          ...
                          <p className="font-semibold text-slate-800 text-sm">
                            {formatQuantity(reagent.effective_monthly_usage)}
                          </p>
                        </div>
                        <div className="bg-slate-50 p-1.5 rounded-lg">
                          <Label className="text-xs font-medium text-slate-600 block">
                          ...
                          <p className="font-semibold text-slate-800 text-sm">
                            {isFinite(reagent.months_of_stock)
                              ? reagent.months_of_stock?.toFixed(1)
                              : "∞"}
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-1">
                            <Input
                              ...
                              onChange={(e) =>
                                handleQuantityChange(reagent.id, e.target.value)
                              }
                            ...
                            {reagent.has_temporary_orders && (
                              <TooltipProvider>
                                <Tooltip>
                                  ...
                                  >
                                    <p className="text-sm">
                                      כמות מוצעת ללא התחשבות בדרישות זמניות:{" "}
                                      {
                                        reagent.suggested_order_quantity_without_temp
                                      }
                                    </p>
                ...
                <div className="min-w-full">
                    ...
                    {columns.map((column) => (
                      <ResizableHeader key={column.id} column={column}>
                        {column.id === "selection" ? (
                          <Checkbox
                            checked={
                              selectedReagents.size > 0 &&
                              selectedReagents.size ===
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 w-full justify-center hover:text-amber-600"
                          >
                            <TableHeaderTooltip
                  ...
                  <div className="divide-y divide-slate-100">
                    {(filteredReagents || []).map((row) => {
                      const netAvailable =
                        (row.available_framework_quantity || 0) -
                        (row.quantity_in_pending_withdrawals || 0) -
                        (row.quantity_in_delivery || 0);
                      return (
                        <div
                          key={row.id}
                          className={`flex items-center transition-colors hover:bg-slate-50 ${
                            [...selectedReagents].some(
                              (item) => item.id === row.id,
                            )
                              ? "bg-amber-50"
                              : ""
                        ...
                        >
                          {columns.map((column) => (
                            ...
                            >
                              ...
                              {column.id === "name" && (
                                <div className="font-semibold text-slate-800 text-right w-full">
                                    ...
                                    {row.has_temporary_orders && (
                                      <TooltipProvider>
                                        <Tooltip>
                                          ...
                                          >
                                            <p className="text-sm">
                                              כמות מוצעת מתחשבת בדרישות רכש
                                              זמניות (ללא מספר קבוע)
                                            </p>
                                          </TooltipContent>
                                        </Tooltip>
                                      </TooltipProvider>
                                  ...
                                  <div className="text-xs text-slate-500 font-normal">
                                    <SmartTooltip
                                      content={`${row.catalog_number} | ${row.supplier?.name || ""}`}
                                    >
                                      {row.catalog_number} |{" "}
                                      {row.supplier?.name || ""}
                                    </SmartTooltip>
                              ...
                              {column.id === "effective_monthly_usage" && (
                                <div>
                                  {formatQuantity(row.effective_monthly_usage)}
                                </div>
                              )}
                              {column.id === "total_quantity_all_batches" && (
                                <div>
                                  {formatQuantity(
                                    row.total_quantity_all_batches,
                                  )}
                                </div>
                              )}
                              {column.id === "months_of_stock" && (
                                <div>
                                  {isFinite(row.months_of_stock)
                                    ? row.months_of_stock?.toFixed(1)
                                    : "∞"}
                                </div>
                              )}
                                ...
                                <Popover>
                                  <PopoverTrigger asChild>
                                    <button
                                      ...
                                      disabled={
                                        !(
                                          row.available_framework_quantity ||
                                          row.quantity_in_pending_withdrawals ||
                                          row.quantity_in_delivery
                                        )
                                      }
                                    >
                                      {formatQuantity(netAvailable)}
                                  ...
                                  {row.has_temporary_orders && (
                                    <TooltipProvider>
                                      <Tooltip>
                                        ...
                                        >
                                          <p className="text-sm">
                                            כמות מוצעת ללא התחשבות בדרישות
                                            זמניות:{" "}
                                            {
                                              row.suggested_order_quantity_without_temp
                                            }
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm p-3 border-t grid grid-cols-2 gap-3 z-20">
          ...
          <Button
            onClick={() => createDocument("order")}
            disabled={selectedReagents.size === 0}
            className="bg-amber-500 hover:bg-amber-600 h-12"
          >
            <FileDown className="h-4 w-4 ms-2" /> צור דרישה
          </Button>
        </div>
      ...
      <Dialog open={showOrderTypeDialog} onOpenChange={setShowOrderTypeDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              בחר את סוג דרישת הרכש שברצונך ליצור.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <Button
              onClick={() => handleOrderCreation("immediate_delivery")}
              className="bg-teal-600 hover:bg-teal-700"
            >
              פתוחה (מיידי)
            </Button>
            <Button
              onClick={() => handleOrderCreation("framework")}
      ...
      >
        <AlertDialogContent className="max-w-2xl" dir="rtl">
          ...
          <AlertDialogFooter className="flex-col sm:flex-row gap-2">
            <AlertDialogCancel
              onClick={handleCancelMissingDetails}
