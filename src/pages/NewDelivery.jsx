...
const DeliveryItemRow = ({
  ...
  const requiresExpiryDate =
    selectedReagentDetails?.requires_expiry_date !== false;
  ...
  return (
    <div
      key={item.key}
      className={`p-4 border rounded-lg relative mb-3 ${
        item.approved
          ? "bg-green-50 border-green-200"
          : isLinked
            ? "bg-blue-50 border-blue-200"
            : selectedReagentDetails?.category === "consumables"
              ? "bg-yellow-50 border-yellow-200" // Consumables might have a different color
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium">
            {item.reagent_name || "בחר פריט..."}
          </span>
          {item.quantity_ordered_snapshot !== null && (
            <span className="text-sm text-gray-600">
              (הוזמן: {formatQuantity(item.quantity_ordered_snapshot)} | נותר:{" "}
              {formatQuantity(item.quantity_remaining_snapshot)})
            </span>
          )}
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="space-y-2 lg:col-span-2">
                ...
                {item.reagent_name && isLinked ? (
                  <div
                    className={`w-full p-2 bg-gray-100 border rounded text-right ${itemValidation.reagent_id ? "border-2 border-red-500" : ""}`}
                  >
                    {item.reagent_name}
                  </div>
                ) : (
                  <Button
                    ...
                    onClick={() => {
                      if (!deliveryData.supplier) {
                        toast({
                          ...
                          description:
                            "בחר ספק בפרטי המשלוח כדי לראות את הריאגנטים הזמינים",
                          variant: "destructive",
                        });
                        return;
                      }
                    ...
                    disabled={isLinked}
                  >
                    {item.reagent_id && item.reagent_name
                      ? item.reagent_name
                      : "בחר פריט..."}
                  </Button>
                )}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  max={
                    isLinked &&
                    typeof item.quantity_remaining_available === "number" &&
                    item.quantity_remaining_available > 0
                      ? item.quantity_remaining_available
                      : undefined
                  }
                  value={item.quantity_received || ""}
                  onChange={(e) =>
                    updateItem(item.key, "quantity_received", e.target.value)
                  }
                ...
                {isLinked &&
                  ...
                  item.quantity_remaining_available > 0 && (
                    <div className="text-xs text-gray-500 mt-1">
                      צפוי לקבלה:{" "}
                      {formatQuantity(item.quantity_remaining_available)}
                    </div>
                  )}
              </div>
              ...
              <Button
                ...
                title={
                  item.approved
                    ...
                    : "אשר פרטי הפריט והוסף לרשימת הפריטים שהתקבלו"
                }
              >
                {item.approved ? (
                  <>
...
export default function NewDeliveryPage() {
  // Renamed component as per outline
  ...
  const { lockSystem, updateLockProgress, clearSystemLock, isLocked } =
    useSystemLock();
    ...
    async (
      ...
      try {
        ...
        if (withdrawalId) {
          ...
          if (withdrawal) {
            const withdrawalItems = await safeFetch(
              () =>
                WithdrawalItem.filter({ withdrawal_request_id: withdrawalId }),
              "withdrawal items",
            );
            ...
            prefilledItemsMapped = withdrawalItems
              ...
              .map((item) => {
                const remaining =
                  (item.quantity_requested || 0) -
                  (item.quantity_received || 0);
                const reagent = reagentsData.find(
                  (r) => r.id === item.reagent_id,
                );
                return {
          ...
          if (order) {
            const orderItems = await safeFetch(
              () =>
                OrderItem.filter({
                  order_id: orderId,
            ...
            prefilledDeliveryForm = {
              ...
              order_number:
                ...
                order.purchase_order_number_sap ||
                "",
            ...
            prefilledItemsMapped = orderItems
              ...
              .map((item) => {
                ...
                return {
                  ...
                  quantity_received: remaining,
                  reagent_catalog_id:
                    item.reagent_catalog_id_snapshot ||
                    reagent?.catalog_item_id ||
                    "",
                  linked_item_id: item.id,
                  quantity_remaining_available: remaining,
                  isNewItem: false,
  ...
  const initPage = useCallback(async () => {
    ...
    try {
      ...
      const [reagentsData, suppliersData, ordersData, withdrawalsData] =
        await Promise.all([
        ...
        if (item.key === keyToUpdate) {
          ...
          if (field === "quantity_received") {
            ...
            if (isNaN(numericValue)) {
              ...
              if (item.isPreFilled) {
                const maxQuantity =
                  item.quantity_remaining_available || Infinity;
  ...
  const handleBarcodeScan = useCallback(
    async (rawData) => {
      try {
        const res = await apiClient.post("/barcode/parse", { rawData });
        const parsed = res.data || res;
        const { productCode, lotNumber, expiryDate, quantity } = parsed;

        let matchedReagent = null;
        if (productCode && reagents?.length) {
          matchedReagent = reagents.find(
            (r) => r.catalog_number === productCode || r.sku === productCode,
          );
        }
        ...
        setItems((prev) => [
          ...prev,
          {
            ...
            coaFile: null,
            coaFileName: "",
            category: matchedReagent?.category || "",
            unit_cost: matchedReagent?.unit_cost || null,
            notes: "",
            approved: false,
          },
        ]);
        ...
        if (matchedReagent) {
          toast({
            ...
            description: `נמצא: ${matchedReagent.name} | אצווה: ${lotNumber || "—"}`,
          });
        } else {
          toast({
            title: "לא נמצא ריאגנט תואם",
            description: `קוד מוצר: ${productCode || "לא זוהה"}. הפריט נוסף — בחר ריאגנט ידנית.`,
            variant: "destructive",
          });
        }
      } catch (err) {
  ...
  const handleLinkedSourceSelection = useCallback(
    async (value) => {
      setDeliveryData((prev) => ({
        ...prev,
        ...
        if (type === "order") {
          ...
          if (selectedOrderData) {
            setDeliveryData((prev) => ({
              ...
              order_number:
                selectedOrderData.order_number_temp ||
                selectedOrderData.order_number_permanent ||
                selectedOrderData.purchase_order_number_sap,
              delivery_type: "with_order",
              linked_withdrawal_id: "",
            }));

            try {
              const orderItemsData = await safeFetch(
                () =>
                  OrderItem.filter({
                    order_id: id,
                    line_status: { $in: ["open", "partially_received"] },
                ...
                const preFilledDeliveryItems = itemsWithRemaining.map(
                  (orderItem) => {
                    ...
                    return {
                      ...
                      quantity_received: remaining,
                      reagent_catalog_id:
                        orderItem.reagent_catalog_id_snapshot ||
                        reagent?.catalog_item_id ||
                        "",
          ...
          const selectedWithdrawalData = activeWithdrawals.find(
            (wr) => wr.id === id,
          );
          if (selectedWithdrawalData) {
            setDeliveryData((prev) => ({
              ...
              order_number:
                selectedWithdrawalData.framework_order_number_snapshot || "",
              delivery_type: "withdrawal",
            ...
            try {
              const withdrawalItemsData = await safeFetch(
                () => WithdrawalItem.filter({ withdrawal_request_id: id }),
                "withdrawal items for linking",
              );
              ...
              if (itemsWithRemaining.length > 0) {
                const preFilledDeliveryItems = itemsWithRemaining.map(
                  (withdrawalItem) => {
                    const remaining =
                      (withdrawalItem.quantity_requested || 0) -
                      (withdrawalItem.quantity_received || 0);
                    const reagent = reagents.find(
                      (r) => r.id === withdrawalItem.reagent_id,
                    );
                    return {
                      ...
                      isNewItem: false,
                      isPreFilled: true,
                      quantity_ordered_snapshot:
                        withdrawalItem.quantity_requested,
                      quantity_remaining_snapshot: remaining,
                      key: Math.random(),
                      coaFile: null,
                      coaFileName: "",
                ...
                toast({
                  ...
                  description:
                    "בקשת המשיכה המקושרת ריקה או שכל הפריטים בה כבר התקבלו.",
                  variant: "default",
                });
                setItems([]);
              }
  ...
  const handleApproveConfirmation = () => {
    ...
    setItems((prev) =>
      prev.map((it) =>
        it.key === itemToApprove.key
          ? {
              ...it,
              approved: true,
              unit_cost: 0,
              notes: `${it.notes || ""} (נקלט ללא עלות עבון חוסר יתרה)`.trim(),
            }
          : it,
      ),
    );
  ...
  const toggleApprove = (keyToToggle) => {
    ...
    if (item.approved) {
      setItems((prev) =>
        prev.map((it) =>
          it.key === keyToToggle ? { ...it, approved: false } : it,
        ),
      );
      ...
      if (
        ...
        setItems((prev) =>
          prev.map((it) =>
            it.key === keyToToggle ? { ...it, approved: true } : it,
          ),
        );
  ...
  const updateWithdrawalStatus = useCallback(async (withdrawalId) => {
    try {
      ...
      if (linkedWithdrawal) {
        const updatedWithdrawalItems = await WithdrawalItem.filter({
          withdrawal_request_id: withdrawalId,
        }).catch(() => []);
        if (Array.isArray(updatedWithdrawalItems)) {
          const allItemsFullyDelivered = updatedWithdrawalItems.every(
            (wi) =>
              wi.line_status === "delivered" ||
              wi.line_status === "cancelled" ||
              wi.line_status === "rejected",
          );
          const anyItemsPartiallyDelivered = updatedWithdrawalItems.some(
            (wi) => wi.line_status === "partially_delivered",
          );
          ...
          if (allItemsFullyDelivered) {
            ...
            updatedWithdrawalItems.every(
              (wi) => (wi.quantity_received || 0) === 0,
            )
          ) {
          ...
          if (newWithdrawalStatus !== linkedWithdrawal.status) {
            await WithdrawalRequest.update(linkedWithdrawal.id, {
          ...
          if (linkedWithdrawal.framework_order_id) {
            try {
              const frameworkOrder = await Order.get(
                linkedWithdrawal.framework_order_id,
              );
              if (frameworkOrder) {
                const frameworkOrderItems = await OrderItem.filter({
                  order_id: frameworkOrder.id,
                });

                let totalFrameworkReceivedMap = new Map();
                const allWithdrawalsForFramework =
                  await WithdrawalRequest.filter({
                    framework_order_id: frameworkOrder.id,
                  }).catch(() => []);
                for (const wr of allWithdrawalsForFramework) {
                  const wrItems = await WithdrawalItem.filter({
                    withdrawal_request_id: wr.id,
                  }).catch(() => []);
                  for (const wri of wrItems) {
                    totalFrameworkReceivedMap.set(
                      wri.reagent_id,
                      (totalFrameworkReceivedMap.get(wri.reagent_id) || 0) +
                        (wri.quantity_received || 0),
                    );
                  }
                }

                for (const foi of frameworkOrderItems) {
                  const newFrameworkQuantityReceived =
                    totalFrameworkReceivedMap.get(foi.reagent_id) || 0;
                  const newFrameworkQuantityRemaining = Math.max(
                    0,
                    (foi.quantity_ordered || 0) - newFrameworkQuantityReceived,
                  );
                  ...
                  if (
                    newFrameworkQuantityReceived !==
                      (foi.quantity_received || 0) ||
                    newFrameworkLineStatus !== foi.line_status
                  ) {
                ...
                const allUpdatedFrameworkItems = await OrderItem.filter({
                  order_id: frameworkOrder.id,
                });

                const allFrameworkItemsFullyReceived =
                  allUpdatedFrameworkItems.every(
                    (foi) =>
                      foi.line_status === "fully_received" ||
                      foi.line_status === "cancelled",
                  );
                const anyFrameworkItemsReceived = allUpdatedFrameworkItems.some(
                  (foi) =>
                    foi.line_status === "partially_received" ||
                    foi.line_status === "fully_received",
                );

                let newFrameworkOrderStatus = frameworkOrder.status;
                if (
                  allFrameworkItemsFullyReceived &&
                  frameworkOrder.status !== "fully_received"
                ) {
                  newFrameworkOrderStatus = "fully_received";
                } else if (
                  anyFrameworkItemsReceived &&
                  frameworkOrder.status === "approved"
                ) {
                  newFrameworkOrderStatus = "partially_received";
                }

                if (newFrameworkOrderStatus !== frameworkOrder.status) {
                  await Order.update(frameworkOrder.id, {
                    status: newFrameworkOrderStatus,
                  });
                }
              }
            } catch (frameworkError) {
              console.error(
                `[NewDelivery] Error updating framework order for withdrawal ${withdrawalId}:`,
                frameworkError,
              );
            }
          }
      ...
      console.error(
        `Error updating withdrawal status for ID ${withdrawalId}:`,
        err,
      );
    }
  }, []);
      ...
      if (linkedOrder) {
        ...
        if (Array.isArray(updatedOrderItems)) {
          const allItemsFullyReceived = updatedOrderItems.every(
            (oi) =>
              oi.line_status === "fully_received" ||
              oi.line_status === "cancelled",
          );
          const anyItemsReceived = updatedOrderItems.some(
            (oi) =>
              oi.line_status === "partially_received" ||
              oi.line_status === "fully_received",
          );
          ...
          if (
            ...
            linkedOrder.status !== "fully_received"
          ) {
  ...
  const handleSaveDelivery = async (isFinal = true) => {
    ...
    if (!deliveryData.supplier) {
      toast({
        title: "שדה חובה חסר",
        description: "יש לבחור ספק למשלוח.",
        variant: "destructive",
      });
      setSaving(false);
      clearSystemLock();
    ...
    try {
      ...
      let hasPartialItems = itemsToProcess.some(
        (item) =>
          item.isPreFilled &&
      ...
      for (const [index, item] of itemsToProcess.entries()) {
        try {
          ...
          updateLockProgress(
            20 + index * progressStep,
            `מעבד פריט ${index + 1}/${itemsToProcess.length}: ${item.reagent_name}`,
          );
          ...
          if (!reagentData) {
            try {
              ...
              throw new Error(
                `לא ניתן למצוא נתוני ריאגנט עבור ${item.reagent_name}`,
              );
            }
          }
          ...
          if (!reagentData.catalog_item_id) {
            reagentData.catalog_item_id =
              reagentData.id || `fallback_${item.reagent_id}`;
          }
          ...
          if (existingBatches.length > 0) {
            ...
            const batchData = {
              ...
              expiry_date: requiresExpiry
                ? item.expiry_date ||
                  format(addYears(new Date(), 100), "yyyy-MM-dd")
                : null,
          ...
          deliveryItemsToCreate.push({
            ...
            linked_source_type: actualLinkedOrderId
              ? "order"
              : actualLinkedWithdrawalId
                ? "withdrawal"
                : null,
          ...
          if (actualLinkedWithdrawalId) {
            // Only update if it's actually linked to a withdrawal
            const withdrawalItem = await WithdrawalItem.get(
              item.linked_item_id,
            );
            if (withdrawalItem) {
              ...
              const isFullyReceived =
                newQuantityReceived >= (withdrawalItem.quantity_requested || 0);

              await WithdrawalItem.update(item.linked_item_id, {
                quantity_received: newQuantityReceived,
                line_status: isFullyReceived
                  ? "delivered"
                  : "partially_delivered",
              });
            }
          } else if (actualLinkedOrderId) {
            // Only update if it's actually linked to an order
            ...
            if (orderItem) {
              const newQuantityReceived =
                ...
                parseFloat(item.quantity_received.toString());
              const isFullyReceived =
                newQuantityReceived >= (orderItem.quantity_ordered || 0);

              await OrderItem.update(item.linked_item_id, {
                quantity_received: newQuantityReceived,
                quantity_remaining: Math.max(
                  0,
                  (orderItem.quantity_ordered || 0) - newQuantityReceived,
                ),
                line_status: isFullyReceived
                  ? "fully_received"
                  : "partially_received",
          ...
          errors.push(
            `שגיאה בעיבוד ${item.reagent_name}: ${itemError.message}`,
          );
        }
      }
      ...
      try {
        for (const reagentId of affectedReagentIds) {
          const response = await updateReagentInventory({
            reagentId,
  ...
  const filteredReagents = useMemo(() => {
    ...
    let relevantReagents = reagents.filter((r) => {
      const supplierName =
        typeof r.supplier === "string" ? r.supplier : r.supplier?.name;
      const supplierId = r.supplier_id || r.supplierId || r.supplier?.id;
      const matchesId =
        supplierId && supplierValue && supplierId === supplierValue;
      const matchesName =
        supplierName && supplierName.trim().toLowerCase() === supplierNameValue;
      return matchesId || matchesName;
    });

    if (searchTerm) {
      const searchLower = searchTerm.toLowerCase();
      relevantReagents = relevantReagents.filter(
        (reagent) =>
          (reagent?.name && reagent.name.toLowerCase().includes(searchLower)) ||
          ((reagent?.catalog_number || reagent?.catalogNumber) &&
            (reagent?.catalog_number || reagent?.catalogNumber)
              .toLowerCase()
    ...
    if (type === "order") {
      ...
      if (order)
        return {
          type: "Order",
          object_type: "Order",
          order_number_temp: order.order_number_temp,
      ...
      if (withdrawal)
        return {
          ...
          supplier_snapshot: withdrawal.supplier_snapshot,
          id: withdrawal.id,
        };
    }
    return null;
  ...
  return (
    <div className="p-2 sm:p-4 md:p-6" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mb-4 gap-2">
          ...
          <h1 className="text-lg md:text-xl font-bold">
            ...
            {preFilledSourceInfo && (
              <span className="text-lg font-normal text-blue-600 me-2">
                ...
                :
                {preFilledSourceInfo.object_type === "Order"
                  ? preFilledSourceInfo.order_number_temp ||
                    preFilledSourceInfo.order_number_permanent
        ...
        <div className="hidden sm:flex items-center gap-2">
          ...
          <Button
            ...
            } // Disabled if no items are approved for final save
          >
            {saving ? (
              ...
              <div>
                ...
                <div className="mt-2 flex gap-2">
                  <Badge variant="outline" className="text-blue-700">
                    ספק:{" "}
                    {preFilledSourceInfo.object_type === "Order"
                      ? preFilledSourceInfo.supplier
                      : preFilledSourceInfo.supplier_snapshot}
                  </Badge>
                  <Badge variant="outline" className="me-2 text-blue-700">
                    {items.filter((item) => item.isPreFilled).length} פריטים
                    נטענו
                  </Badge>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                  ...
                  <div className="space-y-2 sm:col-span-1">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        handleDeliveryDataChange("supplier", value)
                      }
                  ...
                  <div className="space-y-2 sm:col-span-1">
                    ...
                    <DateField
                      ...
                      onChange={(val) =>
                        handleDeliveryDataChange("delivery_date", val)
                      }
                  ...
                  <div className="space-y-2 sm:col-span-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        title="צרף תעודת משלוח (קובץ או צילום)"
                      >
                        <Label
                          htmlFor="delivery_certificate"
                  ...
                  <div className="space-y-2 sm:col-span-2">
                    ...
                    <div className="flex gap-2">
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleDeliveryDataChange("delivery_type", value)
                        }
                      ...
                      {(deliveryData.delivery_type === "with_order" ||
                        deliveryData.delivery_type === "withdrawal") && (
                        <Select
                            ...
                            {activeWithdrawals
                              .filter(
                                (wr) =>
                                  ...
                                  (wr.status === "submitted" ||
                                    wr.status === "approved" ||
                                    wr.status === "in_delivery"),
                              )
                              .map((wr) => (
                                <SelectItem
                                  key={`withdrawal_${wr.id}`}
                                  value={`withdrawal_${wr.id}`}
                                >
                                  משיכה: {wr.withdrawal_number} -{" "}
                                  {wr.supplier_snapshot || "לא ידוע"}
                                </SelectItem>
                              ))}
                          </SelectContent>
                        </Select>
                  ...
                  <div className="space-y-2 sm:col-span-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleDeliveryDataChange("order_number", e.target.value)
                      }
                  ...
                  <div className="space-y-2 sm:col-span-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        handleDeliveryDataChange("notes", e.target.value)
                      }
          ...
          <ScrollArea className="h-[500px] pe-4">
            <div className="p-4">
              {items.length === 0 ? (
                <NoItemsRow
                  onAddItem={addItem}
                  supplierSelected={!!deliveryData.supplier}
                />
              ) : (
            ...
            {filteredReagents.length === 0 ? (
              ...
              filteredReagents.map((reagent) => {
                ...
                const linkedItem = items.find(
                  (item) => item.isPreFilled && item.reagent_id === reagent.id,
                );
                if (linkedItem) {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-600">
                      ספק:{" "}
                      {typeof reagent.supplier === "string"
                        ? reagent.supplier
                        : reagent.supplier?.name}{" "}
                      | קטלוג: {reagent.catalog_item_id}
                      {reagent.total_quantity_all_batches !== undefined &&
                        ` | מלאי כולל: ${formatQuantity(reagent.total_quantity_all_batches || 0)}`}
                      {quantityInfo && (
      ...
      <AlertDialog
        open={showNoBalanceAlert}
        onOpenChange={setShowNoBalanceAlert}
      >
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
            <AlertDialogTitle>קליטת פריט ללא יתרה</AlertDialogTitle>
            <AlertDialogDescription>
              לא קיימת יתרה פתוחה לקליטה עבור פריט זה (
              {itemToApprove?.reagent_name}).
              <br />
              האם ברצונך לקלוט את הפריט ללא עלות? פעולה זו תעדכן את המלאי אך לא
              תחויב במסמך המקור.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setItemToApprove(null)}>
              ביטול
            </AlertDialogCancel>
            <AlertDialogAction
