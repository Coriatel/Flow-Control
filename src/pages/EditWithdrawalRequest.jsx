  ...
  const handleSave = async () => {
    ...
    try {
      await WithdrawalRequest.update(withdrawalId, {
        urgencyLevel: withdrawalRequest.urgency_level,
        requestedDeliveryDate: withdrawalRequest.requested_delivery_date,
        requesterNotes: withdrawalRequest.requester_notes,
        specialInstructions: withdrawalRequest.special_instructions,
      });
  ...
  const handleDelete = async () => {
    try {
      ...
      if (success) {
        ...
        throw new Error(
          response?.error || response?.data?.error || "Failed to delete",
        );
      }
    } catch (error) {
        ...
        <div className="flex items-center space-x-4 space-x-reverse">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              נוצר:{" "}
              {format(
                parseISO(withdrawalRequest.created_date),
                "dd/MM/yyyy HH:mm",
                { locale: he },
              )}
            </p>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-medium">
                  {withdrawalRequest.supplier_snapshot || "לא צוין"}
                </p>
              </div>
            </div>
                ...
                {isEditMode ? (
                  ...
                  <p className="mt-1 text-sm">
                    {withdrawalRequest.requested_delivery_date
                      ? format(
                          parseISO(withdrawalRequest.requested_delivery_date),
                          "dd/MM/yyyy",
                          { locale: he },
                        )
                      : "לא צוין"}
            ...
            <div>
              ...
              {isEditMode ? (
                ...
                <p className="mt-1 text-sm text-gray-600">
                  {withdrawalRequest.special_instructions ||
                    "אין הוראות מיוחדות"}
                </p>
              )}
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              {withdrawalRequest.linked_delivery_ids &&
              withdrawalRequest.linked_delivery_ids.length > 0 ? (
                <div className="mt-2 space-y-2">
                  {withdrawalRequest.linked_delivery_ids.map(
                    (deliveryId, idx) => (
                      <Link
                        key={deliveryId}
                ...
                <p className="mt-2 text-xs text-gray-500">טרם התקבלו משלוחים</p>
              )}
            </div>

            {/* Receive Delivery Button */}
            {["submitted", "approved", "in_delivery"].includes(
              withdrawalRequest.status,
            ) && (
              <Link
                to={createPageUrl(
                  `NewDelivery?withdrawal_id=${withdrawalRequest.id}`,
                )}
            ...
            <div className="pt-4 border-t">
              ...
              <div className="flex items-center text-xs text-gray-500">
                ...
                תאריך בקשה:{" "}
                {format(
                  parseISO(withdrawalRequest.request_date),
                  "dd/MM/yyyy",
                  { locale: he },
                )}
              </div>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              פעולה זו תמחק את בקשת המשיכה ואת כל הפריטים הקשורים אליה. המחיקה
              היא סופית ולא ניתנת לשחזור.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
