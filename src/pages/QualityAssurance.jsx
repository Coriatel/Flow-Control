...
export default function QualityAssurancePage() {
        ...
        filters: parsedFilters
          ? {
              ...
              dateFrom: parsedFilters.dateFrom
                ? parseISO(parsedFilters.dateFrom)
                : null,
              dateTo: parsedFilters.dateTo
                ? parseISO(parsedFilters.dateTo)
                : null,
              ...
              selectedReagents: Array.isArray(parsedFilters.selectedReagents)
                ? parsedFilters.selectedReagents
                : [],
  ...
  const [selectedReagents, setSelectedReagents] = useState(
    savedState.filters.selectedReagents,
  );
  const [missingCOAOnly, setMissingCOAOnly] = useState(
    savedState.filters.missingCOAOnly,
  );

  const [sortConfig, setSortConfig] = useState(() => {
    try {
      const s = localStorage.getItem("qualityAssurance_sortConfig");
      return s ? JSON.parse(s) : { key: "receipt_date", direction: "desc" };
    } catch {
      return { key: "receipt_date", direction: "desc" };
    }
  });
  useEffect(() => {
    localStorage.setItem(
      "qualityAssurance_sortConfig",
  ...
  const initialVisibleColumnsState = useMemo(
    () => createInitialVisibleColumnsState(initialColumns),
    [],
  );
  const [visibleColumns, setVisibleColumns] = useState(
    savedState.columns || initialVisibleColumnsState,
  );
  ...
  const handleCOAUpdate = (itemId, coaData) => {
    setData((prevData) =>
      prevData.map((item) =>
        item.id === itemId ? { ...item, coa_documents: coaData.coas } : item,
      ),
    );
  };
  ...
  const handleCOAUploadSubmit = async () => {
    ...
    try {
      ...
      if (uploadResult?.file_url) {
        ...
        const updatedCoaDocumentsArray = [
          ...existingCoaDocuments,
          newCOADocument,
        ];
  ...
  const handleCOAView = useCallback(
    (item) => {
      if (item.coa_documents && item.coa_documents.length > 0) {
        ...
        if (
          urlToOpen.includes("base44.app/api/apps/") &&
          urlToOpen.includes("/files/")
        ) {
  ...
  const handleDeleteRow = async (batchId) => {
    if (!confirm("האם אתה בטוח שברצונך למחוק אצווה זו? פעולה זו בלתי הפיכה."))
      return;

    try {
      await ReagentBatch.delete(batchId);
      setData((prevData) =>
        prevData.filter((item) => item.reagent_batch_id !== batchId),
      );
      toast({ title: "האצווה נמחקה בהצלחה" });
    } catch (error) {
      toast({
        title: "שגיאה במחיקת האצווה",
        description: error.message,
        variant: "destructive",
      });
    }
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = data.filter((item) => {
      ...
      const matchesDateFrom =
        !dateFrom || (itemExpiryDate && itemExpiryDate >= dateFrom);
      const matchesDateTo =
        !dateTo || (itemExpiryDate && itemExpiryDate <= dateTo);

      const matchesStatus =
        selectedStatuses.length === 0 || selectedStatuses.includes(item.status);
      const matchesReagent =
        selectedReagents.length === 0 ||
        selectedReagents.includes(item.reagent_name);
      const matchesMissingCOA =
        !missingCOAOnly ||
        (missingCOAOnly &&
    ...
    if (sortConfig.key) {
      filtered.sort((a, b) => {
        ...
        let comparison = 0;
        if (
          ["expiry_date", "receipt_date", "first_use_date"].includes(
            sortConfig.key,
          )
        ) {
  ...
  const handlePrint = () => {
    ...
    const totalReceiptQuantity = filteredAndSortedData.reduce(
      (sum, item) => sum + (item.receipt_quantity || 0),
      0,
    );
    const totalStatusQuantity = filteredAndSortedData.reduce(
      (sum, item) => sum + (item.status_quantity || 0),
      0,
    );
        ...
        <body>
          ...
          <table class="print-table">
            <thead>
              <tr>
                ${currentColumns
                  .filter((c) => c.isVisible && c.accessor !== "actions")
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">
                  מציג {filteredBatches.length} מתוך {batches.length} אצוות
                </p>
              </div>
            </div>
                ...
                <PopoverContent className="w-[400px]" align="start">
                  <div className="space-y-4" dir="rtl">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {availableStatuses.map((statusObj) => (
                          <Button
                            key={statusObj.value}
                            variant={
                              selectedStatuses.includes(statusObj.value)
                                ? "default"
                                : "outline"
                            }
                            size="sm"
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-200">
                      <label
                        htmlFor="missing-coa-switch"
                        className="text-sm font-medium text-slate-700"
                      >
                        רק אצוות ללא תעודת אנליזה
                      </label>
                      <Switch
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {initialColumns
                  ...
                  .map((column) => (
                    <label
                      key={column.accessor}
                      className="flex items-center gap-2 text-sm cursor-pointer py-1 px-2 rounded-md hover:bg-slate-100 transition-colors"
                    >
                      <Checkbox
                      ...
                      />
                      <Label
                        htmlFor={`col-select-${column.accessor}`}
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      className="pe-10 bg-white border-slate-300"
                    />
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    {initialColumns
                      ...
                      .map((column) => (
                        <label
                          key={column.accessor}
                          className="flex items-center gap-2 text-sm cursor-pointer py-2 px-3 rounded-md hover:bg-slate-50 transition-colors bg-white border border-slate-200"
                        >
                          <Checkbox
                            checked={visibleColumns[column.accessor]}
                            onCheckedChange={() =>
                          ...
                          />
                          <Label
                            htmlFor={`mobile-col-${column.accessor}`}
                            className="flex-1 text-slate-700 cursor-pointer"
                          >
                            {column.Header}
                          </Label>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-lg font-semibold text-purple-700 mb-1">
                            {coaUploadDialog?.reagent_name}
                          </p>
                          <p className="text-base font-mono font-medium text-slate-700">
                            אצווה: {coaUploadDialog?.batch_number}
                          </p>
                        </div>
                        <Button
                          variant="ghost"
