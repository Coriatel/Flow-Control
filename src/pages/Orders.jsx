...
export default function OrdersPage() {
  ...
  const [sortField, setSortField] = useState(
    () => localStorage.getItem("orders_sortField") || "order_date",
  );
  const [sortDirection, setSortDirection] = useState(
    () => localStorage.getItem("orders_sortDirection") || "desc",
  );
  useEffect(() => {
    localStorage.setItem("orders_sortField", sortField);
    localStorage.setItem("orders_sortDirection", sortDirection);
  ...
  const allColumns = [
    ...
    {
      key: "total_quantity_remaining",
      label: "יתרה",
      defaultWidth: 80,
      sortable: true,
    },
    {
      key: "expected_delivery_start_date",
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      if (success) {
        ...
        throw new Error(
          response?.data?.error || response?.error || "Failed to fetch orders",
        );
      }
    } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (succeeded.length > 0) {
        setOrders((prev) =>
          prev.filter((order) => !succeeded.includes(order.id)),
        );
        setSelectedOrderIds((prev) => {
          const next = new Set(prev);
    ...
    (order, columnKey) => {
      switch (columnKey) {
        ...
        case "linked_withdrawals":
          if (
            !order.linked_withdrawal_request_ids ||
            order.linked_withdrawal_request_ids.length === 0
          ) {
          ...
          return (
            <div className="flex flex-wrap gap-1">
              {order.linked_withdrawal_request_ids
                .slice(0, 2)
                .map((withdrawalId, idx) => {
                  const withdrawal = order.linked_withdrawals?.find(
                    (w) => w.id === withdrawalId,
                  );
                  return (
                    <Link
        ...
        case "linked_deliveries":
          return order.linked_delivery_numbers &&
            order.linked_delivery_numbers.length > 0 ? (
            <div className="flex flex-wrap gap-1">
                            ...
                            ${visibleColumns
                              .filter(
                                (col) =>
                                  col !== "selection" &&
                                  col !== "actions" &&
                                  col !== "linked_withdrawals" &&
                                  col !== "linked_deliveries",
                              )
                              .map((col) => {
                ...
                <div className="flex justify-between items-start gap-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start">
                      <DropdownMenuItem
                        onClick={() =>
                          navigate(createPageUrl(`EditOrder?id=${order.id}`))
                        }
                      >
                ...
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-gray-900">
                      {order.supplier_name_snapshot || "לא צוין"}
                    </span>
                  </div>
