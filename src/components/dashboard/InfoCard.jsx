...
const InfoCard = ({
  ...
  const IconElement = icon
    ? React.cloneElement(icon, { className: `h-5 w-5 ${scheme.iconColor}` })
    : null;

  return (
    <Card
      className={`bg-white shadow-sm border ${scheme.headerBorder} rounded-xl overflow-hidden hover:shadow-lg transition-shadow`}
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span
            className={`text-3xl font-extrabold tabular-nums ${scheme.countColor} leading-none`}
          >
            {count}
          </span>
          {titleLinkTo ? (
            ...
            <span className={`text-sm font-semibold ${scheme.titleColor}`}>
              {title}
            </span>
          )}
        </div>
                      ...
                      >
                        ...
                        {row.actionLinkTo && (
                          <Link
                            ...
                            className="flex-shrink-0 p-1 rounded hover:bg-green-100 text-green-600 hover:text-green-700 transition-colors"
                          >
