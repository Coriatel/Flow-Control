...
interface CriticalAction {
  ...
  text: string;      // תיאור הפעולה
  link: string;      // שם הדף (לא URL מלא)
}
```

## Calculation Logic (Backend)

הפעולות מחושבות ב-`getDashboardData`:

```javascript
const criticalActions = [];

// Check 1: Expired today
if (expiredToday.length > 0) {
  criticalActions.push({
    priority: 'critical',
    text: `דרוש טיפול מיידי ב-${expiredToday.length} ריאגנטים שפג תוקפם`,
    link: 'BatchAndExpiryManagement'
  });
}

// Check 2: Inventory count overdue
if (daysSinceCount > 45) {
  criticalActions.push({
    priority: 'critical',
    text: `יש לבצע ספירת מלאי - האחרונה לפני ${daysSinceCount} ימים`,
    link: 'InventoryCount'
  });
}

// Sort by priority
criticalActions.sort((a, b) => {
